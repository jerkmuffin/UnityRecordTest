using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.UI;
using UnityEngine.Audio;

// what if you cloned another child in audio mixer - that way each child could get their own signal
[RequireComponent (typeof (recorder))]

public class monitor : MonoBehaviour
{
    Text txt;

    private float startRecordingTime;
    AudioClip recording;
    private AudioSource audioSource;

    public float RmsValue;
    public float DbValue;
    public float PitchValue;

    private const int QSamples = 1024;
    private const float RefValue = 0.1f;
    private const float Threshold = 0.02f;

    float[] _samples;
    private float[] _spectrum;
    private float _fSample;

    public bool isRecordingLocal = false;

    public AudioMixer masterMixer;

    recorder recorderScript;



    public void Start()
    {
      recorderScript = GameObject.Find("Button_1").GetComponent<recorder>();
      txt = GameObject.Find("Textdb").GetComponent<Text>();
      txt.text="FMlllL";
      audioSource = GameObject.FindGameObjectWithTag("Canvas").GetComponent<AudioSource>();
      audioSource.clip = Microphone.Start(null, true, 10, 44100);
      audioSource.loop = true;
      while (!(Microphone.GetPosition(null) > 0)){}
      audioSource.Play();
      _samples = new float[QSamples];
      _spectrum = new float[QSamples];
      _fSample = AudioSettings.outputSampleRate;

      masterMixer.SetFloat ("Volume", -80);
      Debug.Log("xxx " + !isRecordingLocal);
    }

    public void Update()
    {
      AnalyzeSound();
      txt.text = DbValue.ToString("F1") + " dB";
      if (DbValue > -10 && !isRecordingLocal)
      {
        Debug.Log("greater than -10");
        isRecordingLocal = true;
        recorderScript.Record();
        Debug.Log("recordgin......");
      }
      // Debug.Log("RMS: " + RmsValue.ToString("F2"));
      // Debug.Log(DbValue.ToString("F1") + " dB");
      // Debug.Log(PitchValue.ToString("F0") + " Hz");
    }

    public void Restart()
    {
      audioSource.clip = Microphone.Start(null, true, 10, 44100);
      audioSource.loop = true;
      while (!(Microphone.GetPosition(null) > 0)){}
      audioSource.Play();
    }


    public void MicCheck()
    {
        Debug.LogError("farts are a thing that smell bad");
        foreach (var device in Microphone.devices)
        {
            Debug.Log("Name: " + device);
        }
    }

    void AnalyzeSound()
    {
        audioSource.GetOutputData(_samples, 0);
        // GetComponent<AudioSource>().GetOutputData(_samples, 0); // fill array with samples
        int i;
        float sum = 0;
        for (i = 0; i < QSamples; i++)
        {
            sum += _samples[i] * _samples[i]; // sum squared samples
        }
        RmsValue = Mathf.Sqrt(sum / QSamples); // rms = square root of average
        DbValue = 20 * Mathf.Log10(RmsValue / RefValue); // calculate dB
        if (DbValue < -160) DbValue = -160; // clamp it to -160dB min
                                            // get sound spectrum
        audioSource.GetSpectrumData(_spectrum, 0, FFTWindow.BlackmanHarris);
        float maxV = 0;
        var maxN = 0;
        for (i = 0; i < QSamples; i++)
        { // find max
            if (!(_spectrum[i] > maxV) || !(_spectrum[i] > Threshold))
                continue;

            maxV = _spectrum[i];
            maxN = i; // maxN is the index of max
        }
        float freqN = maxN; // pass the index to a float variable
        if (maxN > 0 && maxN < QSamples - 1)
        { // interpolate index using neighbours
            var dL = _spectrum[maxN - 1] / _spectrum[maxN];
            var dR = _spectrum[maxN + 1] / _spectrum[maxN];
            freqN += 0.5f * (dR * dR - dL * dL);
        }
        PitchValue = freqN * (_fSample / 2) / QSamples; // convert index to frequency
    }

}
